package com.yux.interviewgoose.mapper;

import com.yux.interviewgoose.model.entity.QuestionBankQuestion;
import com.baomidou.mybatisplus.core.mapper.BaseMapper;

/**
* @author Yuxuan
* @description Mapper for database operations on the table [question_bank_question(Question Bank and Question relationship)]
* @createDate 2025-04-02 18:34:14
* @Entity com.yux.interviewgoose.model.entity.QuestionBankQuestion
*/
public interface QuestionBankQuestionMapper extends BaseMapper<QuestionBankQuestion> {

}




